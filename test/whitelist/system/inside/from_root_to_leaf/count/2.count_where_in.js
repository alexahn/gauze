const description = `Count a whitelist with a caller where_in filter`;
const context = {
	agent: {
...
const operation_name = "CountWhitelistWithWhereIn";
const operation_source = `query ${operation_name}(
	$whitelist: Whitelist_Query__Where,
	$whitelist_ids: Whitelist_Query__Where_Array,
) {
	count_whitelist(where: $whitelist, where_in: $whitelist_ids) {
		select
		count
	}
}
