...
function Headers({ pathfinder, headers }) {
	...
	const proxyURL = pathfinder.stateToURL("gauze.proxy.proxies", {}, { next: location.href });
	const isGraphActive = state.name === "gauze.system.headers.graph";
	return (
		<div>
			<div className="mb1">
			...
			{headers.map(function (header) {
				...
				return (
					<div key={header.name} className="mb0">
						<Link href={pathfinder.stateToURL("gauze.system.headers.header.list", { header: header.graphql_meta_type.toLowerCase() }, { variables })} push={true}>
