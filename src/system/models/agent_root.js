...
class AgentRootSystemModel extends $kernel.src.models.system.SystemModel {
	...
	cursor_read(context, scope, parameters) {
		...
		const realm = {
			...
			operation: {
				operation: $database.interfaces.graphql.operations.agent_root.CURSOR_READ__AGENT_ROOT__OPERATION__GRAPHQL__INTERFACE__DATABASE,
				operation_name: $database.interfaces.graphql.operations.agent_root.CURSOR_READ_NAME__AGENT_ROOT__OPERATION__GRAPHQL__INTERFACE__DATABASE,
			},
		};
		return self._cursor_read(context, scope, parameters, realm).then(function (data) {
			return {
				nodes: data.data.cursor_read_agent_root.nodes.map(function (row) {
					return row.attributes;
				}),
				page_info: data.data.cursor_read_agent_root.page_info,
			};
		});
	}
	...
	cursor_update(context, scope, parameters) {
		...
		const realm = {
			...
			operation: {
				operation: $database.interfaces.graphql.operations.agent_root.CURSOR_UPDATE__AGENT_ROOT__OPERATION__GRAPHQL__INTERFACE__DATABASE,
				operation_name: $database.interfaces.graphql.operations.agent_root.CURSOR_UPDATE_NAME__AGENT_ROOT__OPERATION__GRAPHQL__INTERFACE__DATABASE,
			},
		};
		return self._cursor_update(context, scope, parameters, realm).then(function (data) {
			return {
				nodes: data.data.cursor_update_agent_root.nodes.map(function (row) {
					return row.attributes;
				}),
				page_info: data.data.cursor_update_agent_root.page_info,
			};
		});
	}
	...
	cursor_delete(context, scope, parameters) {
		...
		const realm = {
			...
			operation: {
				operation: $database.interfaces.graphql.operations.agent_root.CURSOR_DELETE__AGENT_ROOT__OPERATION__GRAPHQL__INTERFACE__DATABASE,
				operation_name: $database.interfaces.graphql.operations.agent_root.CURSOR_DELETE_NAME__AGENT_ROOT__OPERATION__GRAPHQL__INTERFACE__DATABASE,
			},
		};
		return self._cursor_delete(context, scope, parameters, realm).then(function (data) {
			return {
				nodes: data.data.cursor_delete_agent_root.nodes.map(function (row) {
					return row.attributes;
				}),
				page_info: data.data.cursor_delete_agent_root.page_info,
			};
		});
	}
