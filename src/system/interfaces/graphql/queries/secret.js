...
const READ__SECRET__QUERY__GRAPHQL__INTERFACE__SYSTEM = {
	...
	resolve: (source, query_arguments, context) => {
		$kernel.logger.io.LOGGER__IO__LOGGER__KERNEL.write("0", __RELATIVE_FILEPATH, "READ__SECRET__QUERY__GRAPHQL__INTERFACE__SYSTEM.resolve:enter", "source", source);
		...
		return CONTROLLER__SECRET__CONTROLLER__SYSTEM.read(
			...
			return data.map(function (record) {
				return SERIALIZER__SECRET__QUERY__GRAPHQL__INTERFACE__SYSTEM.serialize(source, record);
			});
		});
	},
};
...
const COUNT__SECRET__QUERY__GRAPHQL__INTERFACE__SYSTEM = {
	...
	resolve: (source, query_arguments, context) => {
		$kernel.logger.io.LOGGER__IO__LOGGER__KERNEL.write("0", __RELATIVE_FILEPATH, "COUNT__SECRET__QUERY__GRAPHQL__INTERFACE__SYSTEM.resolve:enter", "source", source);
