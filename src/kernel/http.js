...
const HANDLE_REALM_GRAPHQL__HTTP__KERNEL = function ({ $gauze, database, authenticate, authenticators, schema }, ctx, next) {
	...
	return Promise.any(authenticators.map(function (authenticate) {
		...
		return database.transaction(function (transaction) {
			...
			$gauze.kernel.logger.io.LOGGER__IO__LOGGER__KERNEL.write("2", __RELATIVE_FILEPATH, "request", "unexpected", err);
			ctx.response.status = 500
			ctx.response.body = JSON.stringify({
				status: 500,
				message: "Internal Server Error"
...
export {
	HANDLE_REALM_GRAPHQL__HTTP__KERNEL,
	HANDLE_ENVIRONMENT_GRAPHQL__HTTP__KERNEL
}
