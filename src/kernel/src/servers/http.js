		...
		new Promise(function (resolve, reject) {
							...
							return database_manager
								...
								.catch(function (rollback_err) {
									$gauze.kernel.src.logger.io.LOGGER__IO__LOGGER__SRC__KERNEL.write(
										...
										__RELATIVE_FILEPATH,
										"request",
										"TRANSACTION UNAUTHORIZED FAILED TO REVERT",
										rollback_err,
									);
			...
			.then(function (collection) {
				...
				if (rollback) {
					...
					if (process.env.GAUZE_ENV === "development" || process.env.GAUZE_ENV === "development_monolithic" || process.env.GAUZE_ENV === "development_sharded") {
			...
			// note: next is a function that returns a promise that resolves the next middleware
			.finally(next)
	);
};
