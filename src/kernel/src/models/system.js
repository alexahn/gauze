...
class SystemModel extends Model {
	constructor(root_config, graphql_config) {
		...
		self.auth_cache = new TTLLRUCache(parseInt(process.env.GAUZE_CACHE_SYSTEM_AUTH_SIZE, 10) || 1024, parseInt(process.env.GAUZE_CACHE_SYSTEM_AUTH_TTL, 10) || 8192);
		self.auth_loader = new DataLoader(self._auth_batch, {
			cacheMap: self.auth_cache,
		});
		self.auth_loader.model = self;
		self.model_cache = new TTLLRUCache(parseInt(process.env.GAUZE_CACHE_SYSTEM_MODEL_SIZE, 10) || 1024, parseInt(process.env.GAUZE_CACHE_SYSTEM_MODEL_TTL, 10) || 8192);
