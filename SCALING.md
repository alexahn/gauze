...
TypeScript will give us extra guards, but it doesn't really affect codebase architecture. It's mostly good at finding simple mistakes (like misspellings). If it's easier to maximize the codebase architecture without a type system, then it may be worth it to take the risk of allowing simple mistakes to enter the codebase. Ideally all simple mistakes would be caught by tests, since the codebase architecture is leaning towards being able to test the system end to end.
